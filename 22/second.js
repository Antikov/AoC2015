...
function oneTurn(bossInitialStats, heroInitialStats, Initialbuffs, spendMana, heroTurn, spendManaArray) {
...
//  console.log(bossStats, heroStats, buffs, spendMana, heroTurn);
  ...
  if (bossIsDead(bossStats)) {
    if (spendManaMin > spendMana) {
      //console.log(spendMana, spendManaArray);
    }
  ...
  if (heroTurn) {
    Object.keys(spellsManaCost).forEach(function(spellName) {
      if (spellName == "Magic Missile" && heroStats.mana >= spellsManaCost[spellName]) {
        ...
        bossStats.hp -= 4;
        var newSpendManaArray = spendManaArray.slice();newSpendManaArray.push([bossStats, heroStats, spellsManaCost[spellName]]);
        oneTurn(bossStats, heroStats, buffs, spendMana + spellsManaCost[spellName], false, newSpendManaArray);
        bossStats.hp += 4;
        heroStats.mana += spellsManaCost[spellName];
      }
    ...
    if (heroIsDead(heroStats)) {
      ...
      oneTurn(bossStats, heroStats, buffs, spendMana, true, spendManaArray);
    }
  }
}
